...
export default function Home({ params }: { params: { id: String } }) {
  ...
  useEffect(() => {
    if (products.length > 0) {
      const groceriesProduct = products.find(
        (item) => item.id.toLocaleString() === params.id
      );
      setgroceries(groceriesProduct || null);
    }
  }, [products, params.id]);
  ...
  useEffect(() => {
    const calcPrice = () => {
      if (
        groceries?.discountPercentage !== undefined &&
        groceries?.price !== undefined
      ) {
        const discount =
          (groceries?.discountPercentage / 100) * groceries?.price;
        const mainPrice = groceries?.price - discount;
        setPrice(Math.round((mainPrice + Number.EPSILON) * 100) / 100);
      } else if (groceries?.price !== undefined) {
        setPrice(Math.round((groceries.price + Number.EPSILON) * 100) / 100);
      }
    };
    calcPrice();
  }, [groceries]);
        ...
        <div className="flex xl:flex-row flex-col gap-10">
          <div className=" bg-gray-200 rounded-lg w-[350px] xl:w-fit mx-auto flex xl:flex-col xl:justify-start xl:items-start justify-center items-center xl:h-fit ">
            <div className="text-black mt-5 mx-5  ">
              .{groceries?.availabilityStatus}
            </div>

            <img
              src={groceries?.thumbnail}
              alt={groceries?.title}
          ...
          <div className="flex flex-col text-center xl:text-left px-10 gap-5 pt-5">
            ...
            <div className="text-base">
              {renderStars(groceries?.rating)} {groceries?.rating}
            </div>
            ...
            <p className="font-bold">
              Brand:{" "}
              <span className="font-normal capitalize">{groceries?.brand}</span>
            </p>
            <p className="font-bold">
              SKU:{" "}
              <span className="font-normal capitalize">{groceries?.sku}</span>
            </p>
                ...
                <p className="text-black mx-auto xl:mx-0 max-w-[600px]">
                  <ul className="">
                    ...
                    <li className="font-bold">
                      ...
                      <span className="font-light">
                        {groceries?.warrantyInformation}
                      </span>
                    </li>
                    <li className="font-bold">
                      Shipping Information:
                      <span className="font-light">
                        {groceries?.shippingInformation}
                      </span>
                    </li>
                    <li className="font-bold">
                      Return policy:
                      <span className="font-light">
                        {groceries?.returnPolicy}
                      </span>
                    </li>
                    <li className="font-bold">
                      Minimum Order Quantity :
                      <span className="font-light">
                        {groceries?.minimumOrderQuantity}
                      </span>
                    </li>
                  </ul>
                </p>
                  ...
                  <div className=" mx-auto xl:mx-0 text-black">
                    <div>
                      <div className=" grid xl:grid-cols-3 grid-cols-1 gap-4 xl:space-x-10">
                        {groceries?.reviews.map((review: Review, index) => {
                          return (
                            <div key={index}>
                              <div>
